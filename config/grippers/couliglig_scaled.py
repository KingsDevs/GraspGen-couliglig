...
# Uniform scale-up factor. Must match scale_objects.py, the grasp-transform scale
# in convert_datagen_to_graspgen.py, and the length fields in couliglig_scaled.yaml.
SCALE = 7.0


class GripperModel(object):
    """Assembles end_effector + two fingers from the Couliglig URDF, then scales
    the whole mesh by SCALE so it matches the scaled control points / objects.

    The mesh is used for (a) visualization and (b) the discriminator's
    collision-based negative-grasp generation, both of which must be at the same
    scale as the scaled objects. Control points used by the generator come from
    the scaled width/depth in couliglig_scaled.yaml, not from this mesh.
    """
    ...
    def __init__(self, data_root_dir=None):
        ...
        # Bake the uniform scale into the assembled mesh.
...
def get_transform_from_base_link_to_tool_tcp():
    """TCP used ONLY by the dataset's grasp-visibility filter
    (filter_grasps_by_point_cloud_visibility): a grasp is kept if this point lands
    within ~3 cm of the rendered point cloud.

    GraspGen's default is translation([0, 0, depth]) = the FINGERTIP. For Couliglig
    the fingertips overshoot the grasped object by depth - closing_z (≈15 cm scaled),
    so with the default TCP every object is denylisted with code 110 ("No grasps are
    in the visible point cloud"). The object actually sits at the closing-region
    centre, so we point the TCP there instead (matches closing_regions.translation in
    couliglig_scaled.yaml, ×SCALE). Verified: this raises the fraction of grasps
    landing within 3 cm of the object surface from ~2-15% to ~25-90%, so objects are
    kept. Does not affect control points, the training loss, or inference.
    """
    return tra.translation_matrix([0.0, -0.0139 * SCALE, 0.0824 * SCALE])


def get_gripper_offset_bins():
    """Offset bins spanning [0, width]. Used by M2T2 only.

    The width here must stay in sync with the YAML 'width' field (scaled ×SCALE).
    """
    width = 0.0195 * SCALE
