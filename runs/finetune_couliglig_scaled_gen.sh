#!/usr/bin/env bash
# Fine-tune the Couliglig generator FROM the pretrained Robotiq 2F-140 GraspGen
# weights (trained on thousands of objects). Identical architecture + kappa, so
# the weights load drop-in; we only adapt the diffusion head to the Couliglig
# grasp distribution. This inherits Robotiq's object encoder, which our 15-object
# dataset can't learn on its own.
#
# Differs from runs/train_couliglig_scaled_gen.sh only in: checkpoint=pretrained,
# train.finetune=True, separate LOG_DIR, fewer epochs, frequent saves.
# Run from repo root after `source .venv/bin/activate`.

export NGPU=1
export NWORKER=4
export NEPOCH=2000            # fine-tune needs far fewer than from-scratch 5000
export BATCH=8
export PRINT_FREQ=10
export PLOT_FREQ=50
export SAVE_FREQ=200          # frequent: pick the best checkpoint by valid metrics
...
export LOG_DIR="$RESULTS_DIR/logs/${GRIPPER_NAME}_gen_finetune"
export CACHE_DIR="$RESULTS_DIR/cache"
# Initialize from the pretrained Robotiq 2F-140 generator (drop-in: same arch+kappa)
export CHECKPOINT="$CODE_DIR/weights/graspgen_robotiq_2f_140_gen.pth"
export CONSOLE_LOG="$LOG_DIR/console_log.txt"

echo "Fine-tuning generator for $GRIPPER_NAME from $CHECKPOINT"
rm -rf $LOG_DIR
mkdir -p $LOG_DIR
mkdir -p $CACHE_DIR
